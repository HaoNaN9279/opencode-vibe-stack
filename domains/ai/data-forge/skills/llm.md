...
```bash
uv run --directory <data-forge-路径> python -m data_forge.llm <subcommand> [参数]
```

- `<data-forge-路径>`：DataForge 项目目录的路径（包含 `pyproject.toml` 的目录）
...
```bash
uv run --directory /path/to/data-forge python -m data_forge.llm chat \
  --provider openai \
  --model gpt-4o \
...
```bash
uv run --directory /path/to/data-forge python -m data_forge.llm describe \
  --provider openai \
  --model gpt-4o \
...
| `--provider` | string | ✅ | 提供商名称 |
| `--model` | string | ✅ | 模型名称 |
| `--input-dir` | string | ✅ | 输入目录（包含 prompt `.txt` 文件） |
| `--output-dir` | string | ✅ | 输出目录（生成文件保存位置） |
| `--keyfile` | string | ❌ | KEYFILE 路径 |
...
```bash
uv run --directory /path/to/data-forge python -m data_forge.llm batch-chat \
  --provider deepseek \
  --model deepseek-chat \
  --input-dir ./prompts \
...
| `--provider` | string | ✅ | 提供商名称 |
| `--model` | string | ✅ | 视觉模型名称 |
| `--input-dir` | string | ✅ | 输入目录（包含图片文件） |
| `--output-dir` | string | ✅ | 输出目录（描述 `.txt` 文件保存位置） |
| `--keyfile` | string | ❌ | KEYFILE 路径 |
| `--prompt` | string | ❌ | 描述指令（默认 `Describe this image in detail.`） |
| `--temperature` | number | ❌ | 采样温度（默认 `0.7`） |
| `--max-tokens` | number | ❌ | 最大生成 token 数 |
...
```bash
uv run --directory /path/to/data-forge python -m data_forge.llm batch-describe \
  --provider openai \
  --model gpt-4o \
  --input-dir ./images \
  --output-dir ./captions \
...
```bash
uv run --directory /path/to/data-forge python -m data_forge.llm list-providers \
  --keyfile /path/to/keys.json
```

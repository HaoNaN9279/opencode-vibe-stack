...
DataForge 包含 7 个独立工具模块，每个工具的详细用法见对应参考文档：
...
```
resize → [remove-bg] → [convert] → Ollama / LLM → caption → ComfyUI
```
...
4. **`Ollama` 或 `LLM`** — 使用本地模型或云端大模型为图片生成描述文本
5. **`caption`** — 管理、清理、批量编辑生成的标注文件
6. **`ComfyUI`** — 使用标注文本作为提示词，生成合成图像

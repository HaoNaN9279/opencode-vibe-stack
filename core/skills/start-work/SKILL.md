...
- Prometheus 已生成计划文件，存储在 `.opencode/jobs/<日期时间（精确到秒）>_<任务名称>/plan.md` 中。
- 计划文件包含明确的执行步骤、验收标准和依赖关系。
- 当前工作目录为项目根目录。

## 核心工作流

1. **查找计划文件** — 扫描 `.opencode/jobs/` 目录，查找最新的或用户指定的计划文件。
2. **加载计划** — 读取 `plan.md`，解析任务步骤、依赖关系和验收标准。
3. **按序执行** — 根据计划定义的顺序和依赖关系，逐步骤执行任务。
4. **跟踪进度** — 在 `.opencode/jobs/<日期时间（精确到秒）>_<任务名称>/` 下记录执行状态，标记已完成步骤。
